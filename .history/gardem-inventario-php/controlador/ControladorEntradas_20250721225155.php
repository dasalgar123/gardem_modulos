...
class ControladorEntradas {
    ...
    // Método para obtener una entrada específica
    ...
    // Método para editar entrada
    public function editarEntrada($id, $datos) {
        try {
            $stmt = $this->pdo->prepare("
                UPDATE productos_entradas SET
                    producto_id = ?,
                    bodega_id = ?,
                    cantidad = ?,
                    motivo = ?,
    ...
    // Método para procesar edición de entrada
    public function procesarEdicionEntrada() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['entrada_id'])) {
            ...
            if ($this->editarEntrada($id, $datos)) {
                echo "<script>alert('Entrada editada exitosamente'); window.location.href='index.php?page=entradas&success=1';</script>";
            } else {
                echo "<script>alert('Error al editar entrada'); window.location.href='index.php?page=entradas&error=Error al editar entrada';</script>";
            }
        }
    }
}
