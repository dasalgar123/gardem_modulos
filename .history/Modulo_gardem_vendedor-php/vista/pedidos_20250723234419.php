...
// ========================================
// Incluir el controlador de pedidos (patrón MVC)
require_once __DIR__ . '/../controlador/ControladorPedidos.php';

// Crear instancia del controlador con conexión a base de datos
$controlador = new ControladorPedidos($pdo);

// ========================================
// PROCESAMIENTO DE FILTROS Y DATOS
// ========================================
...
// Crear array de filtros para el controlador
$filtros = [
    'search' => $search,      // Búsqueda por texto (cliente, teléfono, correo, productos)
    'date_from' => $date_from, // Fecha de inicio para filtro de rango
    'date_to' => $date_to      // Fecha de fin para filtro de rango
];

// ========================================
// OBTENCIÓN DE DATOS DESDE EL CONTROLADOR
// ========================================
// Obtener pedidos filtrados desde la base de datos
$pedidos = $controlador->obtenerPedidos($filtros);

// Calcular estadísticas en tiempo real
$estadisticas = $controlador->calcularEstadisticas($pedidos);

// Extraer valores de estadísticas para uso en la vista
$total_pedidos = $estadisticas['total_pedidos'];     // Total de pedidos encontrados
$total_ventas = $estadisticas['total_ventas'];       // Suma total de ventas
$promedio_pedido = $estadisticas['promedio_pedido']; // Promedio por pedido
?>
            ...
            <div class="card bg-info text-white mb-2">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        ...
                        <div class="align-self-center"><i class="fas fa-chart-bar fa-2x"></i></div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <form method="GET" action="index.php">
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" id="search" name="search"  placeholder="Cliente, teléfono, correo, productos..." value="<?php echo htmlspecialchars($search); ?>"></div>
                    <div class="col-md-2">
                        <label for="date_from" class="form-label">Desde</label>
                        <input type="date" class="form-control" id="date_from" name="date_from" value="<?php echo htmlspecialchars($date_from); ?>"></div>
                    <div class="col-md-2">
                        <label for="date_to" class="form-label">Hasta</label>
                        <input type="date" class="form-control" id="date_to" name="date_to" value="<?php echo htmlspecialchars($date_to); ?>"></div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2">
                        <label class="form-label">&nbsp;</label>
                        <a href="index.php?page=pedidos" class="btn btn-secondary w-100"><i class="fas fa-times"></i> Limpiar</a>
                    </div>
                </div>
            </form>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover align-middle">
                        ...
                        <?php foreach ($pedidos as $pedido): ?>
                            <tr>
                                <td><span class="badge bg-secondary">#<?php echo $pedido['id']; ?></span></td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        </div><strong><?php echo htmlspecialchars($pedido['nombre_cliente']); ?></strong></div>
                                </td>
                                <td><i class="fas fa-phone text-muted me-1"></i><?php echo htmlspecialchars($pedido['telefono']); ?></td>
                                <td style="max-width: 250px;">
                                    <div class="productos-preview">
                                        ...
                                        if (count(explode('•', $pedido['productos'])) > 2) { echo '<small class="text-muted">... y más</small>';}
                                    ?>
                                    </div>
                                </td>
                                <td><span class="badge bg-success fs-6">$<?php echo number_format($pedido['total'], 0, ',', '.'); ?></span</td>
                                <td>
