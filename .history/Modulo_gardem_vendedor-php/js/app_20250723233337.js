...
let ventasModule = {
    ...
    crearFilaProducto: function(numeroProducto) {
        ...
        fila.innerHTML = `
            <div class="row g-2">
                ...
                <div class="col-md-1"><input type="number" class="form-control cantidad-input" name="productos[${numeroProducto}][cantidad]" placeholder="Cant." min="1" required></div>
                <div class="col-md-2"><input type="number" class="form-control precio-input" name="productos[${numeroProducto}][precio]" placeholder="Precio" step="0.01" readonly></div>
                <div class="col-md-1"><input type="number" class="form-control subtotal-input" name="productos[${numeroProducto}][subtotal]" placeholder="Total" step="0.01" readonly></div>
                <div class="col-md-1"><button type="button" class="btn btn-danger btn-sm remover-producto" title="Eliminar producto"><i class="fas fa-trash"></i></button></div>
            </div>
        `;
    ...
    setupRowEvents: function(fila) {
        ...
        if (select) { select.addEventListener('change', this.calcularPrecioYSubtotal.bind(this)); }
        if (cantidad) { cantidad.addEventListener('input', this.calcularPrecioYSubtotal.bind(this)); }
    },
    ...
    handleChange: function(e) { if (e.target.classList.contains('producto-select')) { this.calcularPrecioYSubtotal(e.target); } },
    handleInput: function(e) { if (e.target.classList.contains('cantidad-input')) { this.calcularPrecioYSubtotal(e.target); } },
    ...
    renumerarProductos: function() {
        ...
        productos.forEach((producto, index) => { producto.setAttribute('data-producto-id', index + 1); });
    },
    
    obtenerOpcionesProductos: function() {
        const primerSelect = document.querySelector('.producto-select');
        if (primerSelect) { return primerSelect.innerHTML.replace(/name="productos\[\d+\]\[producto_id\]"/g, ''); }
        return '';
    },
    
    obtenerOpcionesColores: function() {
        const primerSelect = document.querySelector('select[name*="[color_id]"]');
        if (primerSelect) { return primerSelect.innerHTML.replace(/name="productos\[\d+\]\[color_id\]"/g, ''); }
        return '';
    },
    
    obtenerOpcionesTallas: function() {
        const primerSelect = document.querySelector('select[name*="[talla_id]"]');
        if (primerSelect) { return primerSelect.innerHTML.replace(/name="productos\[\d+\]\[talla_id\]"/g, '');}
        return '';
    },
    ...
    calcularPrecioYSubtotal: function(elemento) {
        ...
        if (select && select.value) {
            ...
            if (precioSeleccionado && precio) { precio.value = precioSeleccionado;}
        }
        ...
        } else if (subtotal) {subtotal.value = '';}
    ...
    cancelarVenta: function() {
        ...
        if (container) {
            ...
            for (let i = 1; i < productos.length; i++) { productos[i].remove();}
...
function removerProducto(elemento) {
    ...
    if (productoCounter > 1) {
        ...
        if (productoCounter === 1) { document.querySelector('.remover-producto').style.display = 'none';}
 ...
 */
function limpiarFiltros() { window.location.href = 'index.php?page=productos';}
 ...
 */
function editarProducto(id) { window.location.href = 'index.php?page=productos&action=editar&id=' + id;}
...
function validarFormularioProducto() {
    ...
    if (!tipo) { alert('Debe seleccionar un tipo de producto'); return false;}
 ...
 */
function initializeClientes() { console.log('Inicializando funcionalidades de clientes');}
...
/**
 * Inicializa funcionalidades específicas de inventario
 */
function initializeInventario() {
    ...
    if (!inventarioTable) {
        console.log('No se encontró inventarioTable - no estamos en la página de inventario');
        return;
    }
    
    console.log('✅ Estamos en la página de inventario');
    
    // Configurar botones de exportación
    const btnExcel = document.querySelector('button[onclick="exportToExcel()"]');
    const btnPDF = document.querySelector('button[onclick="exportToPDF()"]');
    const btnPrint = document.querySelector('button[onclick="printInventario()"]');
    ...
    if (btnPrint) {
        btnPrint.addEventListener('click', printInventario);
        console.log('✅ Botón Imprimir configurado');
    }
    
    console.log('✅ Funcionalidades de inventario inicializadas correctamente');
}

/**
 * Exporta el inventario a Excel
 * Utiliza la librería XLSX para generar archivo Excel
 */
function exportToExcel() {
    ...
    try {
        ...
        if (!table) {
            ...
            showNotification('Error: No se encontró la tabla de inventario', 'error');
            return;
        }
        ...
        console.log('✅ Inventario exportado a Excel exitosamente');
        showNotification('Inventario exportado a Excel correctamente', 'success');
 ...
 * Utiliza jsPDF y autoTable para generar archivo PDF
 */
function exportToPDF() {
    ...
    try {
        ...
        console.log('✅ Inventario exportado a PDF exitosamente');
        showNotification('Inventario exportado a PDF correctamente', 'success');
 ...
 * Utiliza la función nativa de impresión del navegador
 */
function printInventario() {
    console.log('🖨️ Imprimiendo inventario...');
    ...
    try {
        ...
        console.log('✅ Inventario enviado a impresión');
        showNotification('Inventario enviado a impresión', 'info');
...
window.exportToExcel = exportToExcel;                 // Exportar a Excel
window.exportToPDF = exportToPDF;                     // Exportar a PDF
window.printInventario = printInventario;             // Imprimir inventario
