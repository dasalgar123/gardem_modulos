...
// Consulta para mostrar cada variante por separado (talla y color individuales)
$query = "
    SELECT 
        p.id,
        p.nombre,
        ...
        pe.talla,
        pe.color,
        pe.cantidad as entradas,
        COALESCE(ps.total_salidas, 0) as total_salidas,
        (pe.cantidad - COALESCE(ps.total_salidas, 0)) as saldo,
        CASE 
            WHEN (pe.cantidad - COALESCE(ps.total_salidas, 0)) = 0 THEN 'Agotado'
            WHEN (pe.cantidad - COALESCE(ps.total_salidas, 0)) < 5 THEN 'Stock Bajo'
            ELSE 'Disponible'
        END as estado
    FROM productos p
    INNER JOIN productos_entradas pe ON p.id = pe.producto_id
    LEFT JOIN (
                        ...
                        <?php else: ?>
                            <?php foreach ($inventario as $item): ?>
                                <tr>
                                    ...
                                    <td><span class="badge bg-secondary"><?php echo htmlspecialchars($item['talla'] ?? 'N/A'); ?></span></td>
                                    <td><span class="badge bg-primary"><?php echo htmlspecialchars($item['color'] ?? 'N/A'); ?></span></td>
                                    <td><span class="badge bg-primary"><?php echo number_format($item['entradas']); ?></span></td>
                                    <td><span class="badge bg-warning"><?php echo number_format($item['total_salidas']); ?></span></td>
                                    <td><span class="badge bg-dark"><?php echo number_format($item['saldo']); ?></span></td>
                                    <td>
                                        <?php 
                                        $estado_class = match($item['estado']) {
                                        ...
                                        ?>
                                        <span class="badge <?php echo $estado_class; ?>"><?php echo $item['estado']; ?></span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
