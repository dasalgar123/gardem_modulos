...
if (isset($_POST['editar_id'])) {
    ...
    $stmt = $pdo->prepare("UPDATE productos_entradas SET producto_id=?, cantidad=?, bodega_id=?, fecha=?, motivo=?, beneficiario_tipo=?, beneficiario_id=?, factura_remision=?, beneficiario=?, color_id=?, talla_id=?, referencia=? WHERE id=?");
    $stmt->execute([$producto_id, $cantidad, $bodega_id, $fecha, $motivo, $beneficiario_tipo, $beneficiario_id, $factura_remision, $beneficiario, $color_id, $talla_id, $referencia, $id]);
...
try {
    $stmt = $pdo->query("SELECT pe.id, pe.producto_id, pe.cantidad, pe.fecha, pe.motivo, pe.beneficiario, pe.factura_remision, 
        p.nombre as producto, pe.referencia, c.nombre as color, t.nombre as talla
            ...
            <form method="POST" action="index.php?page=entradas">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" name="referencia" value="<?php echo htmlspecialchars($entrada_editar['referencia'] ?? ''); ?>">
                    </div>
                    <div class="col-md-4">
                        ...
                        <?php else: ?>
                            <?php foreach ($entradas as $entrada): ?>
                                <tr>
                                    ...
                                    <td><span class="badge bg-light text-dark"><?php echo !empty($entrada['referencia']) ? $entrada['referencia'] : 'N/A'; ?></span></td>
